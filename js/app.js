...
$("#frmList").submit(function(event) {
 
    var zaman=new Date() 
    var date = zaman.getDate() + "." + (zaman.getMonth()+1) + "." + zaman.getFullYear() + "-" + zaman.getHours()+":"+zaman.getMinutes()+":"+zaman.getSeconds();

    event.preventDefault();

    let todo = {
        id: maxId() + 1,
        task: $("#txtListe").val().trim(),
        isDone: false,
        datetime: date
    }
...
function listTodos() {
    ...
    $.each(sortedTodos, function(index, todo) {
        ...
        li.append(cb);
        li.append(" " + todo.task);
        li.append("<br/>"+"<p style='font-size:12px;color:white;'>"+todo.datetime+"</p>")
        $("ul#todos").append(li);
    });
}
